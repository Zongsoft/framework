...
namespace Zongsoft.Net
{
	public class TcpServerChannelManager<T> : SocketServer, IReadOnlyCollection<TcpServerChannel<T>>, IDisposable
	{
		...
		internal ValueTask<Zongsoft.Common.OperationResult> HandleAsync(TcpServerChannel<T> channel, in T package, CancellationToken cancellation) => this.Server.Handler?.HandleAsync(channel, package, cancellation) ?? ValueTask.FromResult(Zongsoft.Common.OperationResult.Fail());
		#endregion
