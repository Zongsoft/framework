...
public class PluginLoader
{
	...
	internal void Load(PluginOptions options)
	{
		...
		if(!Directory.Exists(options.PluginsPath))
			throw new DirectoryNotFoundException(string.Format(Properties.Resources.Plugin_DirectoryNotFound_Message, options.PluginsPath));
	...
	private Plugin LoadPluginManifest(string filePath, Plugin parent, PluginOptions options)
	{
		...
		if(parent == null)
		{
			if(_topmosts.Any(p => string.Equals(p.Name, plugin.Name, StringComparison.OrdinalIgnoreCase)))
				throw new PluginFileException(plugin.FilePath, string.Format(Properties.Resources.Plugin_DuplicateName_Message, plugin.Name, plugin.FilePath));
	...
	private void LoadPluginContent(Plugin plugin, PluginOptions options)
	{
		...
		catch(Exception ex)
		{
			...
			throw new PluginFileException(plugin.FilePath, string.Format(Properties.Resources.Plugin_FileResolveFailed_Message, plugin.FilePath), ex);
		}
	}
			...
			foreach(var dependency in plugin.Manifest.Dependencies)
			{
				if(dependency.Plugin == null)
				{
					dependency.Plugin = this.FindDependency(dependency.Name) ??
						throw new PluginException(string.Format(Properties.Resources.Plugin_DependencyNotFound_Message, plugin.Name, dependency.Name));
				}
