...
internal class PluginResolver
{
	...
	public void ResolvePluginWithoutManifest(Plugin plugin)
	{
		...
		using(XmlReader reader = GetReader(plugin.FilePath))
		{
			...
			//如果读取插件文件失败则退出
			if(!reader.Read())
				throw new PluginException(string.Format(Properties.Resources.Plugin_FileUnreadable_Message, plugin.FilePath));
			...
			if(!string.Equals(reader.Name, "plugin"))
				throw new PluginException(string.Format(Properties.Resources.Plugin_FileFormatInvalid_Message, plugin.FilePath));
			...
			while(reader.Read())
			{
				...
				switch(reader.Name)
				{
					...
					case "extension":
						...
						break;
					default:
						throw new PluginFileException(string.Format(Properties.Resources.Plugin_ElementInvalid_Message, reader.Name));
				}
			}
		}
	}
	...
	public Plugin ResolvePluginManifestOnly(string filePath, Plugin parent)
	{
		using(XmlReader reader = GetReader(filePath))
		{
			...
			//如果读取插件文件失败则退出
			if(!reader.Read())
				throw new PluginException(string.Format(Properties.Resources.Plugin_FileUnreadable_Message, filePath));
			...
			//判断插件文件的跟节点名称是否为“plugin”
			if(!string.Equals(reader.Name, "plugin"))
				throw new PluginException(string.Format(Properties.Resources.Plugin_FileFormatInvalid_Message, filePath));
			...
			while(reader.NodeType == XmlNodeType.Element)
			{
				switch(reader.Name)
				{
					...
					default:
						...
						throw new PluginException(string.Format(Properties.Resources.Plugin_ElementUndefined_Message, reader.Name));
				}
			}
	...
	private static void ResolveManifest(XmlReader reader, Plugin plugin)
	{
		...
		while(reader.Read() && reader.Depth > depth)
		{
			switch(reader.Name)
			{
				...
				case "dependencies":
					if(plugin.IsHidden)
						throw new PluginFileException(string.Format(Properties.Resources.Plugin_HiddenDependenciesInvalid_Message, plugin.FilePath));

					ResolveDependencies(reader.ReadSubtree(), plugin);
					break;
				default:
					throw new PluginFileException(string.Format(Properties.Resources.Plugin_ManifestElementInvalid_Message, reader.Name));
			}
		}
	}
	...
	private void ResolveExtendedElement(XmlReader reader, Plugin plugin, string path)
	{
		...
		if(node == null)
			throw new PluginException(string.Format(Properties.Resources.Plugin_ExtensionNotFound_Message, path, parts[0], plugin.FilePath));
	...
	private void ResolveExtendedElement(XmlReader reader, Builtin builtin)
	{
		...
		if(parts.Length != 2)
			throw new PluginException(string.Format(Properties.Resources.Builtin_ExtensionInvalid_Message, reader.Name, builtin));
			...
			switch(parts[1].ToLowerInvariant())
			{
				case "constructor":
					if(builtin.BuiltinType == null)
						throw new PluginException(string.Format(Properties.Resources.Builtin_ExtensionTypeRequired_Message, reader.Name));
		...
		if(!reader.IsEmptyElement)
		{
			...
			if(reader.Read() && reader.Depth > depth)
			{
				if(reader.NodeType == XmlNodeType.Text)
					...
					throw new PluginException(string.Format(Properties.Resources.Builtin_BehaviorChildrenInvalid_Message, behaviorName, builtin.ToString()));
			}
		}
	}
	...
	private static XmlReader GetReader(string filePath)
	{
		...
		if(!File.Exists(filePath))
			throw new FileNotFoundException(string.Format(Properties.Resources.File_NotFound_Message, filePath));
