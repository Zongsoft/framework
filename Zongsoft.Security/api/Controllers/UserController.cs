...
{
	#region 构造函数
	public UserController(IUserService<IUser> service) => this.Service = service;
	#endregion

	#region 公共属性
	public IUserService<IUser> Service { get; }
	#endregion
	...
	public async Task<IActionResult> Get(string identifier, [FromQuery]Paging page = null, CancellationToken cancellation = default)
	{
		...
		var result = await this.Service.GetAsync(new Identifier(typeof(IUser), identifier), this.Request.Headers.GetDataSchema(), cancellation);
		return result != null ? this.Ok(result) : this.NoContent();
	}

	[HttpPost]
	public async Task<ActionResult> Create([FromBody]UserModel model, CancellationToken cancellation = default)
	{
		if(model == null)
	...
	public async Task<IActionResult> Delete(CancellationToken cancellation = default)
	{
		...
		return await this.Service.DeleteAsync(ids.Select(id => new Identifier(typeof(IUser), id)), cancellation) > 0 ? this.NoContent() : this.NotFound();
	}

	[HttpPut("{id:required}")]
	[HttpPatch("{id:required}")]
	public async Task<IActionResult> Update(uint id, [FromBody]UserModel model, CancellationToken cancellation = default)
	{
		if(id == 0)
			return this.BadRequest();
	...
	public async Task<IActionResult> Exists(string @namespace, string name)
	{
		...
		return await this.Service.ExistsAsync(new Identifier(typeof(IUser), $"{@namespace}:{name}")) ? this.NoContent() : this.NotFound();
	}
	#endregion
}
