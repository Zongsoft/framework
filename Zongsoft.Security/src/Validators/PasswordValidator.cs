...
public class PasswordValidator : IValidator<string>, IMatchable
{
	...
	public bool Validate(string data, object parameter, Action<string> failure = null)
	{
		...
		{
			case Configuration.PasswordStrength.Digits:
				...
				break;
			case Configuration.PasswordStrength.Lowest:
				...
				break;
			case Configuration.PasswordStrength.Normal:
				...
				break;
			case Configuration.PasswordStrength.Highest:
				isValidate = this.GetStrength(data) == PASSWORD_STRENGTH_DIGIT +
	...
	#region 服务匹配
	bool IMatchable.Match(object parameter) => this.Match(parameter as string);
	public bool Match(string parameter) =>
		string.Equals(parameter, "Password", StringComparison.OrdinalIgnoreCase) |
