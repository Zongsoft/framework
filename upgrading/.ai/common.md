...
每个场景至少完成两轮升级，示例基线版本为 `1.0.0`：

1. 第一轮发布 `1.1.0` 或等价 minor 递增版本的全量包（`Fully`），确认目标程序升级成功。
2. 第二轮发布 `1.1.1` 或等价 patch 递增版本的增量包（`Delta`），确认连续升级成功。
...
7. 第一轮升级：添加 `1.1.0` marker，构建 `Fully` 全量包、publish、验证 S3 对象；观察升级前先停止目标宿主并删除宿主程序的 `bin` 目录，再重新构建和部署干净基线宿主、upgrader 和 deployer，确保全量升级测试干净完整。
8. 第二轮升级：添加 `1.1.1` marker，构建 `Delta` 增量包、publish、验证 S3 对象、观察升级。
...
- `--version` 递增且高于当前运行版本；全量包提升 minor 版本号，增量包只提升 patch（第三部分）版本号。
- `--kind`、平台、架构、框架、源目录和输出目录正确；第一轮必须是 `Fully`，第二轮必须是 `Delta`。
- 第一轮全量升级前必须删除宿主程序的 `bin` 目录，并在干净基线部署上验证全量包能完整恢复目标宿主。
- 输出包和 manifest 实际存在。
