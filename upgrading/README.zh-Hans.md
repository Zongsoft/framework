...
组件 | 包或项目 | 职责
----|---------|-----
应用端升级器 | [Zongsoft.Upgrading.Upgrader](upgrader/README.zh-Hans.md) | 发现新版本、下载升级包、校验校验码、解压文件、写入部署描述文件、启动部署器并退出当前应用程序。
应用端部署器 | [Zongsoft.Upgrading.Deployer](deployer/README.zh-Hans.md) | 一个 Native AOT 可执行程序，负责等待宿主应用退出、将解压后的文件复制到应用目录、执行部署器命令、重启应用并结束自身。
Web 服务端包管理器 | [Zongsoft.Upgrading.Web](web/README.zh-Hans.md) | 托管发布元数据和升级包文件，管理应用、发布、实例，并提供升级器使用的发现和下载接口。
升级包工具 | [Zongsoft.Tools.Upgrader](tool/README.zh-Hans.md) | 制作升级 ZIP 包和 `.manifest` 清单文件，重新计算校验码，并将升级包发布到支持的通道。
...
- [升级器](upgrader/README.zh-Hans.md)
- [部署器](deployer/README.zh-Hans.md)
- [Web端](web/README.zh-Hans.md)
- [工具](tool/README.zh-Hans.md)
