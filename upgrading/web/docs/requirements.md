...
2. 支持维护应用、应用版本名、实例、发布、发布属性、发布执行器和发布跟踪。
...
ReleaseTracing 表示某个发布在某个实例上的升级阶段跟踪，主键为 `ReleaseId + InstanceId`。
...
4. 服务端查询满足发布控制、平台、架构和版本名的候选 Release。
...
2. 客户端或管理端通过 ReleaseTracing API 维护发布在实例上的阶段。
3. 发布跟踪可按 Release 或 Instance 两个维度查询与写入。
...
10. Instance 和 ReleaseTracing 能反映客户端安装实例及其升级阶段。
