...
{
	public class TDengineUpdateStatementVisitor : UpdateStatementVisitor
	{
		#region 单例字段
		public static readonly TDengineUpdateStatementVisitor Instance = new TDengineUpdateStatementVisitor();
		#endregion

		#region 构造函数
		private TDengineUpdateStatementVisitor() { }
		#endregion
		...
		protected override void VisitTables(ExpressionVisitorContext context, UpdateStatement statement, IList<TableIdentifier> tables)
		{
			...
			/*
			 * 注意：由于 TDengine 的 UPDATE 语句不支持 FROM 子句，因此必须将其改写为多表修改的语法。
			 */
				...
				foreach(var source in statement.From)
				{
					switch(source)
					{
						...
						case JoinClause join:
							...
							{
								throw new DataException($"The {TDengineDriver.NAME} driver does not support the FROM clause of the UPDATE statement contain an expression of type '{join.Target.GetType().Name}'.");
							}

							break;
						default:
							throw new NotSupportedException($"The {TDengineDriver.NAME} driver does not support the FROM clause of the UPDATE statement contain an expression of type '{source.GetType().Name}'.");
					}
				}
			}
		}
		...
		{
			/*
			 * 由于 TDengine 的 UPDATE 语句不支持 FROM 子句，故不输出任何内容，且不调用基类同名方法以避免生成错误的语句。
			 */
		}
		#endregion
	}
