...
public class InsertTest(DatabaseFixture database) : IDisposable
{
	...
	public async Task InsertLogAsync()
	{
		...
		log = Model.Build<Log>(log =>
		{
			...
			log.TenantId = 1;
			log.BranchId = 0;
			log.Timestamp = Zongsoft.Common.DateTimeExtension.Reset(
				Zongsoft.Common.DateTimeExtension.Reset(DateTime.UtcNow, Zongsoft.Common.DateTimePart.Millisecond),
				Zongsoft.Common.DateTimePart.Microsecond);
		});
