...
[**Z**ongsoft.**D**ata.**M**ySql](https://github.com/Zongsoft/framework/tree/main/Zongsoft.Data/drivers/mysql) 是 [_**Z**ongsoft_](https://github.com/Zongsoft/framework) 开源框架中的数据引擎的底层驱动，提供了 [_**M**y**SQL**_](https://www.mysql.com) 数据库访问的相关功能，对上层应用透明，只需将该插件库部署到应用程序的插件目录中即可。


## 适用场景

当 Zongsoft 数据模型存储于 MySQL 时选择本包。上层应用仍面向 `IDataAccess`/`IDataService<T>`；本包负责把表达式和数据变更翻译为对应方言，并通过 MySqlConnector 创建连接。

本方言负责 MySQL 标识符、生成值回读、分页、参数和批量导入。服务器 SQL Mode 与字符集会改变验证和比较行为，应保持环境一致。
...
请将 `Zongsoft.Data.MySql.plugin` 与程序集一起部署。清单会在框架工作台中注册连接设置解析器和数据驱动；只有 NuGet 引用而缺少插件清单，不会在插件宿主中激活驱动。
...
`driver` 不区分大小写，但建议使用规范拼写 `MySql`。`value` 由本包的连接设置驱动解析，并最终传给 MySqlConnector；可用关键字请查阅[提供程序文档](https://mysqlconnector.net/)。

> 💡 实体映射、条件、分页、事务和 `IDataAccess` 用法统一参阅 [Zongsoft.Data 指南](../../README.zh-Hans.md)。切换驱动不应迫使应用服务依赖提供程序连接类型。

连接字符串使用[本驱动公开的设置模型](src/Configuration/MySqlConnectionSettings.cs)，不保证支持底层 SDK 的全部关键字。示例仅指向本机测试环境；需要先准备数据库、账号及权限，并替换明确标注的测试凭据。不要把示例当作服务器初始化脚本。
...
- [驱动实现规则](../../AGENTS.md)
- [数据引擎实现技能](../../SKILL.md)
- [MySQL 提供程序文档](https://mysqlconnector.net/)
...
| 文件复制及依赖 | [Zongsoft.Data.MySql.deploy](src/Zongsoft.Data.MySql.deploy) |
...
清单列出的附属产物包括：`Zongsoft.Data.MySql.plugin`。同时保留程序集、依赖与附属资源目录。部署后重启宿主，先检查插件加载与服务/驱动注册，再验证前文的使用流程；不要把“文件已复制”当作“功能已启用”。
