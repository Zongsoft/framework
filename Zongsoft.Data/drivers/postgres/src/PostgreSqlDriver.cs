...
public class PostgreSqlDriver : DataDriverBase
{
	...
	#region 单例字段
	public static readonly PostgreSqlDriver Instance = new();
	#endregion
	...
	public override IStatementBuilder Builder => PostgreSqlStatementBuilder.Default;
	#endregion

	#region 公共方法
	public override Exception OnError(IDataAccessContext context, Exception exception)
	{
		if(exception is NpgsqlException error)
		{
			switch(error.ErrorCode)
			{
				...
				case 1586:
					...
					if(TryGetConstraint(error.Message, out var table, out var key, out var value))
					{
						...
						return new DataConstraintException(NAME, error.ErrorCode, kind, key, value, table, fields);
					}

					return new DataConflictException(NAME, error.ErrorCode, null, null, error);
				case 1216:
				case 1217:
				case 1451:
				case 1452:
	...
	public override DbCommand CreateCommand(string text, CommandType commandType = CommandType.Text) => new NpgsqlCommand(text)
	{
