...
当 Zongsoft 数据模型存储于 PostgreSQL 时选择本包。上层应用仍面向 `IDataAccess`/`IDataService<T>`；本包负责把表达式和数据变更翻译为对应方言，并通过 Npgsql 创建连接。

本方言负责 PostgreSQL 标识符、RETURNING、分页、参数、受支持的数组/自定义类型以及批量导入；Schema 搜索路径与服务器扩展应显式配置。
...
请将 `Zongsoft.Data.PostgreSql.plugin` 与程序集一起部署。清单会在框架工作台中注册连接设置解析器和数据驱动；只有 NuGet 引用而缺少插件清单，不会在插件宿主中激活驱动。
...
`driver` 不区分大小写，但建议使用规范拼写 `PostgreSQL`。`value` 由本包的连接设置驱动解析，并最终传给 Npgsql；可用关键字请查阅[提供程序文档](https://www.npgsql.org/doc/)。

> 💡 实体映射、条件、分页、事务和 `IDataAccess` 用法统一参阅 [Zongsoft.Data 指南](../../README.zh-Hans.md)。切换驱动不应迫使应用服务依赖提供程序连接类型。

连接字符串使用[本驱动公开的设置模型](src/Configuration/PostgreSqlConnectionSettings.cs)，不保证支持底层 SDK 的全部关键字。示例仅指向本机测试环境；需要先准备数据库、账号及权限，并替换明确标注的测试凭据。不要把示例当作服务器初始化脚本。
...
- [驱动实现规则](../../AGENTS.md)
- [数据引擎实现技能](../../SKILL.md)
- [PostgreSQL 提供程序文档](https://www.npgsql.org/doc/)
...
| 文件复制及依赖 | [Zongsoft.Data.PostgreSql.deploy](src/Zongsoft.Data.PostgreSql.deploy) |
...
清单列出的附属产物包括：`Zongsoft.Data.PostgreSql.plugin`。同时保留程序集、依赖与附属资源目录。部署后重启宿主，先检查插件加载与服务/驱动注册，再验证前文的使用流程；不要把“文件已复制”当作“功能已启用”。
