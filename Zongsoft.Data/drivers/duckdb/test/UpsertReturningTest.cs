...
public class UpsertReturningTest(DatabaseFixture database) : IDisposable
{
	...
	public async Task UpsertAsync()
	{
		...
		//注意：DuckDB 的 RETURNING 不支持返回增改前的值，因此下面的断言应该为假。
		Assert.False(options.Returning.Rows[0].TryGetValue(nameof(UserModel.UserId), ReturningKind.Older, out value));
		Assert.False(options.Returning.Rows[0].TryGetValue(nameof(UserModel.Name), ReturningKind.Older, out value));
		Assert.False(options.Returning.Rows[0].TryGetValue(nameof(UserModel.Enabled), ReturningKind.Older, out value));
	...
	public async Task UpsertWithChildrenAsync()
	{
		...
		//注意：DuckDB 的 RETURNING 不支持返回增改前的值，因此下面的断言应该为假。
		Assert.False(options.Returning.Rows[0].TryGetValue(nameof(RoleModel.RoleId), ReturningKind.Older, out value));
		Assert.False(options.Returning.Rows[0].TryGetValue(nameof(RoleModel.Name), ReturningKind.Older, out value));
		Assert.False(options.Returning.Rows[0].TryGetValue(nameof(RoleModel.Enabled), ReturningKind.Older, out value));
	...
	public async Task UpsertManyAsync()
	{
		...
		var rows = options.Returning.Rows.ToDictionary(row =>
		{
		...
		for(int i = 0; i < COUNT; i++)
		{
			...
			Assert.Equal(true, value);
			Assert.False(row.TryGetValue(nameof(UserModel.UserId), ReturningKind.Older, out value));
			Assert.False(row.TryGetValue(nameof(UserModel.Name), ReturningKind.Older, out value));
		...
		Assert.Equal(COUNT, count);
		Assert.Equal(COUNT, options.Returning.Rows.Count);
		rows = options.Returning.Rows.ToDictionary(row =>
		{
			Assert.True(row.TryGetValue(nameof(UserModel.UserId), ReturningKind.Newer, out var value));
			return Convert.ToUInt32(value);
		});
	...
	public async Task UpsertManyWithChildrenAsync()
	{
		...
		for(int i = 0; i < models.Length; i++)
		{
			...
			Assert.Equal(true, value);
			Assert.False(row.TryGetValue(nameof(RoleModel.RoleId), ReturningKind.Older, out value));
			Assert.False(row.TryGetValue(nameof(RoleModel.Name), ReturningKind.Older, out value));
