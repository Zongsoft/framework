...
public class UpdateTest(DatabaseFixture database) : IDisposable
{
	...
	public async Task UpdateAsync()
	{
		...
		Assert.Equal(true, Zongsoft.Common.Convert.ConvertValue(value, typeof(bool)));

		//注意：SQLite 的 RETURNING 不支持返回更新前的值，因此下面的断言应该为假。
		Assert.False(returning.Rows[0].TryGetValue(nameof(UserModel.Name), ReturningKind.Older, out value));
		Assert.False(returning.Rows[0].TryGetValue(nameof(UserModel.Enabled), ReturningKind.Older, out value));
	}

	[Fact(Skip = "Cascading updates are not currently supported.")]
