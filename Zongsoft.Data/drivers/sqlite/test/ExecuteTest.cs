...
public class ExecuteTest(DatabaseFixture database) : IDisposable
{
	...
	public async Task ExecuteAsync()
	{
		...
		var command = Mapping.Commands.Script(SQLiteDriver.NAME, DataCommandMutability.Insert, script)
		...
		command = Mapping.Commands.Script(SQLiteDriver.NAME, script)
			.Parameter("@Severity", DataType.Byte)
		...
		command = Mapping.Commands.Script(SQLiteDriver.NAME, "SELECT * FROM \"Log\"");
		...
		Assert.Equal(timestamp.ToUniversalTime(), log.Timestamp);
		#endif
	}
