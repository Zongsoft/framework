...
public class DeleteStatementBuilder : IStatementBuilder<DataDeleteContext>
{
	...
	protected virtual IEnumerable<IStatementBase> BuildSimplicity(DataDeleteContext context)
	{
		...
		//构建当前实体的继承链的关联集
		Join(context.Aliaser, statement, statement.Table).ToArray();
	...
	private static void Join(Aliaser aliaser, DeleteStatement statement, TableIdentifier table, SchemaMember schema)
	{
		...
		//生成当前导航属性表的继承链关联集
		var joins = Join(aliaser, statement, target, schema.FullPath);
		...
		if(schema.HasChildren)
		{
			foreach(var child in schema.Children)
			{
				...
				Join(aliaser, statement, target, child);
			}
		}
	}
	#endregion
