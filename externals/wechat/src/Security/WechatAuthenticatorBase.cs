...
public abstract class WechatAuthenticatorBase : IAuthenticator, IAuthenticator<string, WechatAuthenticatorBase.Identity>
{
	...
	public async ValueTask<Identity> VerifyAsync(string key, string secret, string scenario, Parameters parameters, CancellationToken cancellation = default)
	{
		...
		if(!Account.TryParse(key, out var account))
			throw new ArgumentException(string.Format(Properties.Resources.Wechat_KeyUnrecognized_Message, key), nameof(key));
	...
	private static async ValueTask<string> GetSecretAsync(object data, CancellationToken cancellation = default)
	{
		...
		if(text == null)
		{
			...
			else if(data is System.IO.Stream stream)
			{
				...
				throw new InvalidOperationException(string.Format(Properties.Resources.Identity_DataTypeUnsupported_Message, data.GetType().FullName));
		}
