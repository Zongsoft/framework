	...
	public partial class PaymentManager
	{
		public class CertificateService
		{
			...
			public async ValueTask<ICertificate> GetCertificateAsync(CancellationToken cancellation = default)
			{
				...
				var response = await HttpClientFactory.GetHttpClient(_authority.Certificate).GetAsync("https://api.mch.weixin.qq.com/v3/certificates", cancellation);
				...
				return _certificate = new Certificate(latest.Value.SerialNo, latest.Value.SerialNo, "RSA", publicKey: GetCertificatePublicKey(_authority, latest.Value.Data))
				{
