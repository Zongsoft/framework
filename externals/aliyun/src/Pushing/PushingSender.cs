...
public class PushingSender
{
	...
	public async Task<PushingResult> SendAsync(string name, string title, string content, string destination, PushingSenderSettings settings, CancellationToken cancellation)
	{
		...
		if(!options.Apps.TryGetValue(name, out var app))
			throw new InvalidOperationException(string.Format(Properties.Resources.Pushing_AppNotFound_Message, name));
	...
	{
		return this.Options ?? throw new InvalidOperationException(Properties.Resources.Pushing_ConfigurationRequired_Message);
	}
	#endregion
}
