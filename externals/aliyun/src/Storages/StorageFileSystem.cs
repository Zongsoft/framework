...
public class StorageFileSystem : Zongsoft.IO.IFileSystem
{
	...
	{
		return this.Options ?? throw new InvalidOperationException(Properties.Resources.Storage_ConfigurationRequired_Message);
	}
	#endregion
	...
	private sealed class StorageDirectoryProvider : IDirectory
	{
		...
		private string EnsureDirectoryPath(string text, out string bucketName)
		{
			...
			if(!path.IsDirectory)
				throw new PathException(Properties.Resources.Directory_PathInvalid_Message);

			if(path.Segments.Length == 0)
				throw new PathException(Properties.Resources.Directory_BucketRequired_Message);
	...
	private sealed class StorageFileProvider : IFile
	{
		...
		private string EnsureFilePath(string text, out string bucketName)
		{
			...
			if(!path.IsFile)
				throw new PathException(Properties.Resources.File_PathInvalid_Message);
