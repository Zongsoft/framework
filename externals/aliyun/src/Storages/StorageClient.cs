...
public class StorageClient
{
	...
	public async ValueTask<StorageSearchResult> SearchAsync(string path, Func<string, string> getUrl, CancellationToken cancellation)
	{
		...
		{
			if(path.Contains('?'))
				throw new ArgumentException(string.Format(Properties.Resources.Path_PatternInvalid_Message, path));

			var index = path.IndexOf('*');

			if(index >= 0 && index < path.Length - 1)
				throw new ArgumentException(string.Format(Properties.Resources.Path_WildcardPositionInvalid_Message, path));
