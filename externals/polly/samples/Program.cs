...
internal class Program
{
	static void Main(string[] args)
	{
		...
		Terminal.Console.Executor.Command("throttle", context =>
		{
			...
			{
				case "token":
					_features = _features.Throttle(permit, queue, ThrottleLimiter.Token(context.Options.GetValue("threshold", 0)));
					break;
				case "fixed":
					_features = _features.Throttle(permit, queue, ThrottleLimiter.Fixed(window));
					break;
				case "sliding":
					_features = _features.Throttle(permit, queue, ThrottleLimiter.Sliding(window, context.Options.GetValue("windowSize", 0)));
					break;
				default:
