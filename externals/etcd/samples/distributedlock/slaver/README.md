# Etcd Lock slaver Process

[English](README.md) | [简体中文](README.zh-Hans.md)

## Purpose

This is the slaver participant in the [two-process lock example](../README.md). It uses the same lock and counter as its peer; it is not a separate lock implementation or a permanent leader/follower role.
...
[Program.cs](Program.cs) constructs `EtcdService`, sets namespace `samples:distributed-lock`, acquires `worker` with 5-second expiry and 2-second renewal, then calls `EnterAsync`. It increases `counter`, prints `SLAVER fence=..., counter=...`, and waits 1 s before scope disposal. There are 10 iterations.
