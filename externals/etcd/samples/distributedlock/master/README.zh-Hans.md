...
这是[双进程锁样例](../README.zh-Hans.md)中的 master 参与者，与另一个进程共用锁及计数器，不是另一套锁实现，也不表示固定主从角色。
...
[Program.cs](Program.cs) 创建 `EtcdService`，设置 `samples:distributed-lock` 命名空间，以 5 秒有效期及 2 秒续期间隔获取 `worker`，再调用 `EnterAsync`。随后递增 `counter`，打印 `MASTER fence=..., counter=...`，等待 750 ms 后离开作用域释放锁，共运行 10 轮。
