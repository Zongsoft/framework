...
public class EtcdLockCommand() : EtcdCommandBase("Lock")
{
	protected override ValueTask<object> OnExecuteAsync(CommandContext context, CancellationToken cancellation)
	{
		var etcd = GetEtcd(context);
		context.Output.WriteLine((etcd.Tokenizer ??= DistributedLockTokenizer.Random).Name);
		return ValueTask.FromResult<object>(etcd);
	}
}
