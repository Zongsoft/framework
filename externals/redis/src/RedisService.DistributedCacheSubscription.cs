...
{
	internal sealed class DistributedCacheSubscription : ChannelBase, IDistributedCacheSubscription
	{
		#region 成员字段
		private readonly RedisService _cache;
		private readonly DistributedCacheNotificationHub _hub;
		...
		#region 构造函数
		public DistributedCacheSubscription(RedisService cache, DistributedCacheNotificationHub hub, IHandler<DistributedCacheNotification> handler, DistributedCacheSubscriptionOptions options)
		{
		...
		protected override async ValueTask OnCloseAsync(CancellationToken cancellation)
		{
			...
			{
				Zongsoft.Diagnostics.Logging.GetLogging(typeof(DistributedCacheSubscription)).Error(exception);
			}
			finally
			{
