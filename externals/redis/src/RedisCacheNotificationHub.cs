...
internal sealed class RedisCacheNotificationHub
{
	private static readonly ConditionalWeakTable<ConnectionMultiplexer, ConcurrentDictionary<Scope, RedisCacheNotificationHub>> _registries = new();

	private readonly Scope _scope;
	private readonly ConcurrentDictionary<Scope, RedisCacheNotificationHub> _registry;
	...
	private RedisCacheNotificationHub(ConnectionMultiplexer connection, int database, string @namespace, ConcurrentDictionary<Scope, RedisCacheNotificationHub> registry)
	{
	...
	internal static async ValueTask<RedisCacheNotificationHub> GetAsync(ConnectionMultiplexer connection, int database, string @namespace, CancellationToken cancellation)
	{
		...
		while(true)
		{
			var hub = registry.GetOrAdd(scope, static (scope, state) => new RedisCacheNotificationHub(state, scope.Database, scope.Namespace, _registries.GetValue(state, static _ => new())), connection);
	...
	internal async ValueTask<bool> AddAsync(RedisCacheSubscription subscription, CancellationToken cancellation)
	{
		...
		try
		{
			...
			if(!_subscriptions.TryAdd(subscription, 0))
				throw new InvalidOperationException("The cache notification subscription is already registered.");
	...
	internal async ValueTask RemoveAsync(RedisCacheSubscription subscription)
	{
	...
	{
		if(!message.TryParseKeyNotification(out var source) || !RedisCacheSubscription.TryGetNotificationKind(source.Type, out var kind))
			return;
