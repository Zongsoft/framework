...
## Result Boundaries and Cleanup

🚨 The master's `reset` clears test state for the selected run. Use a dedicated database and a unique `run-id` each time; never reset while other workers are active, and do not use the demonstration password in a real deployment.

Zero overlaps, expiry overlaps, and stale writes are expected observations, not guarantees under arbitrary network delay, process pauses, or scheduling. A successful `renew` run does not prove indefinite ownership; business storage must validate fencing tokens instead of relying only on the lock object's local state.

Wait for this run's master and all slavers to finish, then use `reset` with the same `run-id` and `scenario` to clean its state. Do not clear the database or another run's prefix. Scheduling and reporting code is in [master/Program.cs](master/Program.cs) and [slaver/Program.cs](slaver/Program.cs). For application-side plugin use through the public lock manager, see the [library guide](../../README.md).
