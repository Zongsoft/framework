...
internal class Program
{
	public static async Task<int> Main(string[] args)
	{
		...
		var splash = CommandOutletContent.Create()
			...
			.AppendLine(CommandOutletColor.Cyan, "Zongsoft Redis Distributed Lock Slaver".Justify(64))
			.AppendLine(CommandOutletColor.Yellow, new string('·', 64));
	...
	private static string GetCommandText(string[] args)
	{
		...
		return text.StartsWith("run", StringComparison.OrdinalIgnoreCase) ? text : $"run {text}";
	}
}
