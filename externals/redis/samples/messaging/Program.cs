...
namespace Zongsoft.Externals.Redis.Messaging.Samples;

internal class Program
{
	static async Task Main(string[] args)
	{
		using var queue = new RedisQueue("Redis",
			Configuration.RedisConnectionSettingsDriver.Instance.GetSettings("Redis",
				$"server=127.0.0.1:6379;password=xxxxxx;group=Zongsoft.Externals.Redis.Samples;client=Sample-{Guid.NewGuid():N};timeout=10s;deadline=3;idleTimeout=30s;"));
		...
		executor.Command("info", context =>
		{
			...
			if(queue.Subscribers.Count > 0)
			{
				...
				{
					var tags = subscriber.Tags.Length == 0 ? "N/A" : string.Join(',', subscriber.Tags);
					context.Output.WriteLine($"[{++index}] {subscriber.Topic} (Tags:{tags})");
				}
			}
		});
		...
		executor.Command("subscribe", async (context, cancellation) =>
		{
			...
			{
				var subscriber = await queue.SubscribeAsync(context.Arguments[i], tags, Handler.Instance, cancellation);
		...
		var splash = CommandOutletContent.Create()
			...
			.AppendLine(CommandOutletColor.Cyan, "Welcome to the Redis Streams Client.".Justify(50))
			.AppendLine(CommandOutletColor.Yellow, new string('·', 50));

		await executor.RunAsync(splash);
	}

	internal sealed class Handler : HandlerBase<Zongsoft.Messaging.Message>
	{
		#region 单例字段
		...
		#region 重写方法
		protected override async ValueTask OnHandleAsync(Zongsoft.Messaging.Message message, Parameters parameters, CancellationToken cancellation)
		{
			if(message.IsEmpty)
				return;
			...
			var content = CommandOutletContent.Create()
				...
				.Append(CommandOutletColor.DarkCyan, " Identifier:")
				.AppendLine(CommandOutletColor.DarkGreen, message.Identifier ?? "N/A")
				.Append(CommandOutletColor.DarkCyan, " Tags:")
				.AppendLine(CommandOutletColor.DarkGreen, message.Tags ?? "N/A")
