...
public readonly struct CellAddress : IEquatable<CellAddress>
{
	...
	public static (CellAddress start, CellAddress end) Range(ReadOnlySpan<char> range)
	{
		...
		if(index < 2 || index >= range.Length - 2)
			throw new ArgumentException(Properties.Resources.Spreadsheet_RangeInvalid_Message);
	...
	private static (int row, int column) ParseCore(ReadOnlySpan<char> text, bool throwOnError = true)
	{
			...
			switch(context.State)
			{
				...
				case State.Column:
					if(DoColumn(ref context, out var value))
					{
						...
						{
							if(throwOnError)
								throw new ArgumentException(string.Format(Properties.Resources.Spreadsheet_ColumnInvalid_Message, value.ToString()));
				...
				case State.Row:
					if(DoRow(ref context, out value))
					{
						...
						{
							if(throwOnError)
								throw new ArgumentException(string.Format(Properties.Resources.Spreadsheet_RowInvalid_Message, value.ToString()));
		...
		if(context.Final(out var span) == State.Row)
		{
			...
			{
				if(throwOnError)
					throw new ArgumentException(string.Format(Properties.Resources.Spreadsheet_RowInvalid_Message, span.ToString()));
