...
{
	private sealed class CronScheduler(JobStorage storage) : IScheduler<TriggerOptions.Cron>, IMatchable, IMatchable<string>
	{
		#region 常量定义
		...
		#region 成员字段
		private readonly RecurringJobManager _scheduler = new(storage);
		#endregion
		...
		public ValueTask<string> ScheduleAsync(string name, TriggerOptions.Cron options, CancellationToken cancellation = default)
		{
			...
			_scheduler.AddOrUpdate(options.Identifier, job, options.Expression, new RecurringJobOptions());
		...
		public ValueTask<bool> RescheduleAsync(string identifier, CancellationToken cancellation = default)
		{
			...
			try
			{
				_scheduler.TriggerJob(identifier);
		...
		public ValueTask<bool> UnscheduleAsync(string identifier, CancellationToken cancellation = default)
		{
			...
			_scheduler.RemoveIfExists(identifier);
			return ValueTask.FromResult(true);
		}
		#endregion
