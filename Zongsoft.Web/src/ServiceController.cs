...
public class ServiceController<TModel, TService> : ServiceControllerBase<TModel, TService> where TService : class, IDataService<TModel>
{
	...
	{
		return this.Content((await this.DataService.CountAsync(null, key, this.OptionsBuilder.Count(), cancellation)).ToString());
	}
	...
	public virtual async ValueTask<IActionResult> CountAsync(CancellationToken cancellation = default)
	{
		...
		var count = await this.DataService.CountAsync(Criteria.Transform(criteria as IModel), this.OptionsBuilder.Count(), cancellation);
