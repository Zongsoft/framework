...
- 异常消息使用中英文资源；资源键以 `.` 分隔，异常消息键以 `.Message` 结尾。含义相同的文本优先复用资源，只在参数上有差异的消息使用完整格式模板。
- 修改资源后用 `ResXFileCodeGenerator`（或采用同一强类型生成器的 ResGen）重新生成 `Resources.Designer.cs`，业务代码通过生成属性访问资源；不手工维护生成属性。
