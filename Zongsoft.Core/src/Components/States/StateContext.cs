...
public class StateContext<TKey, TValue> : IStateContext<TKey, TValue> where TKey : struct, IEquatable<TKey> where TValue : struct
{
	...
	#region 重写方法
	public override string ToString() => $"{this.Key}:{this.State}";
	#endregion
	...
	private class ParameterCollection : IDictionary<object, object>
	{
		...
		public ICollection<object> Keys => _machine.HasParameters ? [.. _parameters.Keys.Union(_machine.Parameters.Keys)] : _parameters.Keys;
		public ICollection<object> Values => _machine.HasParameters ? [.. _parameters.Values.Union(_machine.Parameters.Values)] : _parameters.Values;
		...
		public void Add(KeyValuePair<object, object> item) => _parameters.Add(item);
		public void Clear() => _parameters.Clear();
		public bool Remove(object key) => _parameters.Remove(key) || (_machine.HasParameters && _machine.Parameters.Remove(key));
		public bool Remove(KeyValuePair<object, object> item) => _parameters.Remove(item.Key) || (_machine.HasParameters && _machine.Parameters.Remove(item.Key));
		public bool Contains(KeyValuePair<object, object> item) => _parameters.Contains(item) || (_machine.HasParameters && _machine.Parameters.Contains(item));
		public bool ContainsKey(object key) => _parameters.ContainsKey(key) || (_machine.HasParameters && _machine.Parameters.ContainsKey(key));
		...
		public bool TryGetValue(object key, out object value) => _parameters.TryGetValue(key, out value) || (_machine.HasParameters && _machine.Parameters.TryGetValue(key, out value));
