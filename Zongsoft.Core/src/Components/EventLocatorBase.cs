...
namespace Zongsoft.Components
{
	public abstract class EventLocatorBase<TContext> : IHandlerLocator<TContext> where TContext : class
	{
		...
		private class EventHandler : IHandler
		{
			...
			public bool CanHandle(object request, IEnumerable<KeyValuePair<string, object>> parameters = null) => _descriptor.Handlers.Count > 0;
			...
			public ValueTask HandleAsync(object caller, object request, IEnumerable<KeyValuePair<string, object>> parameters, CancellationToken cancellation = default) => _descriptor.HandleAsync(request, parameters, cancellation);
		}
		#endregion
	}
}