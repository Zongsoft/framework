...
public abstract class CommandBase<TContext> : CommandBase, ICommand<TContext> where TContext : CommandContext
{
	...
	///		<para>如果<seealso cref="CommandBase.Predication"/>属性为空(<c>null</c>)，则返回<seealso cref="CommandBase.Enabled"/>属性值；否则返回由<seealso cref="CommandBase.Predication"/>属性指定的断言对象的断言方法的值。</para>
	/// </remarks>
