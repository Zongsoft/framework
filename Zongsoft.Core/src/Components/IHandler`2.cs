...
namespace Zongsoft.Components
{
	...
	public interface IHandler<in TArgument, TResult> : IHandler
	{
		...
		bool CanHandle(TArgument argument, IEnumerable<KeyValuePair<string, object>> parameters = null);
		...
		ValueTask<TResult> HandleAsync(object caller, TArgument argument, CancellationToken cancellation = default);
		...
		/// <param name="cancellation">指定的异步取消标记。</param>
		/// <returns>返回的异步任务。</returns>
		ValueTask<TResult> HandleAsync(object caller, TArgument argument, IEnumerable<KeyValuePair<string, object>> parameters, CancellationToken cancellation = default);
	}
}
