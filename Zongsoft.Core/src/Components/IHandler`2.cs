...
namespace Zongsoft.Components
{
	...
	public interface IHandler<in TRequest, TResult> : IHandler
	{
		...
		ValueTask<TResult> HandleAsync(object caller, TRequest request, CancellationToken cancellation = default);
		...
		/// <param name="cancellation">指定的异步取消标记。</param>
		/// <returns>返回的异步任务。</returns>
		ValueTask<TResult> HandleAsync(object caller, TRequest request, IEnumerable<KeyValuePair<string, object>> parameters, CancellationToken cancellation = default);
	}
}
