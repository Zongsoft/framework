...
public interface IRoleService<TRole, TMember> where TRole : IRole where TMember : IMember<TRole>
{
	...
	ValueTask<TRole> GetAsync(Identifier identifier, CancellationToken cancellation = default);
	...
	/// <param name="cancellation">指定的异步操作取消标记。</param>
	/// <returns>返回由<paramref name="identifier"/>参数指定的角色对象，如果没有找到则返回空(<c>null</c>)。</returns>
	ValueTask<TRole> GetAsync(Identifier identifier, string schema, CancellationToken cancellation = default);
	...
	IAsyncEnumerable<TRole> FindAsync(string keyword, string schema, Paging paging, CancellationToken cancellation = default);
	...
	/// <param name="cancellation">指定的异步操作取消标记。</param>
	/// <returns>返回找到的角色结果集。</returns>
	IAsyncEnumerable<TRole> FindAsync(ICondition criteria, string schema, Paging paging, CancellationToken cancellation = default);
