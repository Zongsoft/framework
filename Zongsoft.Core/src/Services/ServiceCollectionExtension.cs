	...
	public static class ServiceCollectionExtension
	{
		public static IServiceCollection Register(this IServiceCollection services, Assembly assembly)
		{
			...
			foreach(var type in assembly.GetExportedTypes().Where(t => t.IsClass && !t.IsAbstract))
			{
				...
				var lifetime = string.IsNullOrEmpty(attribute.Provider) ? ServiceLifetime.Singleton : ServiceLifetime.Scoped;
				...
				if(contracts != null)
				{
					...
					{
						services.Add(ServiceDescriptor.Describe(contracts[i], type, lifetime));
					}
				}
