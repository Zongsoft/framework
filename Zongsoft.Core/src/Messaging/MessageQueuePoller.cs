...
namespace Zongsoft.Messaging
{
	...
	{
		#region 私有变量
		private PollerListener _listener;
		...
		#region 公共属性
		public ListenerBase<TMessage> Listener
		{
			get
			{
				if(_listener == null)
				{
					...
					_listener = new PollerListener(this);
				}
		...
		#region 侦听器类
		private class PollerListener : ListenerBase<TMessage>
		{
			#region 成员字段
			private MessageQueuePoller<TMessage> _poller;
			#endregion

			#region 构造函数
			public PollerListener(MessageQueuePoller<TMessage> poller) : base("MessageQueuePoller")
			{
				_poller = poller;
			}
			#endregion
