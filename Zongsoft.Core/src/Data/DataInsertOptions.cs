...
{
	#region 构造函数
	public DataInsertOptions() => this.SequenceRetrievable = true;
	public DataInsertOptions(Collections.Parameters parameters) : base(parameters) => this.SequenceRetrievable = true;
	public DataInsertOptions(IEnumerable<KeyValuePair<string, object>> parameters) : base(parameters) => this.SequenceRetrievable = true;
	#endregion
	...
	/// <param name="names">指定的新增后的返回成员名集。</param>
	/// <returns>返回创建的<see cref="Builder"/>构建器对象。</returns>
	public static Builder Return(params IEnumerable<string> names) => new(null) { Returning = new(ReturningKind.Newer, names) };
	...
	public class Builder : DataMutateOptionsBuilder<DataInsertOptions>
	{
		...
		public Builder Return(params IEnumerable<string> names)
		{
			if(names == null)
				...
				this.Returning = new(ReturningKind.Newer, names);
			else
			{
