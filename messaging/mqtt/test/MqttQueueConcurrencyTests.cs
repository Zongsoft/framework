...
public class MqttQueueConcurrencyTests
{
	#region 常量定义
	...
	public async Task MultipleClientsReceiveEveryConcurrentPublication()
	{
		...
		var publishers = Enumerable.Range(0, PUBLISHER_COUNT)
			.Select(index => MqttTestUtility.CreateQueue(server.Port, $"fanout-publisher-{index}-{identity}"))
			.ToArray();
		var subscribers = Enumerable.Range(0, SUBSCRIBER_COUNT)
			.Select(index => MqttTestUtility.CreateQueue(server.Port, $"fanout-subscriber-{index}-{identity}"))
			.ToArray();
		var audits = Enumerable.Range(0, SUBSCRIBER_COUNT).Select(_ => new MqttMessageAudit(topic)).ToArray();
		var consumers = Array.Empty<MqttSubscriber>();
		...
		try
		{
			...
			Assert.True(throughput >= MINIMUM_DELIVERIES_PER_SECOND,
				$"MQTT fan-out throughput {throughput:F2} deliveries/s was below {MINIMUM_DELIVERIES_PER_SECOND:F2}; {DELIVERY_COUNT} deliveries took {stopwatch.Elapsed}.");
		}
		finally
		{
	...
	public async Task UnsubscribingDuringConcurrentPublishingDoesNotInterruptOtherClients()
	{
		...
		var publishers = Enumerable.Range(0, PUBLISHER_COUNT)
			.Select(index => MqttTestUtility.CreateQueue(server.Port, $"unsubscribe-publisher-{index}-{identity}"))
			.ToArray();
		var subscribers = Enumerable.Range(0, SUBSCRIBER_COUNT)
			.Select(index => MqttTestUtility.CreateQueue(server.Port, $"unsubscribe-subscriber-{index}-{identity}"))
			.ToArray();
		var audits = Enumerable.Range(0, SUBSCRIBER_COUNT).Select(_ => new MqttMessageAudit(topic)).ToArray();
		var consumers = Array.Empty<MqttSubscriber>();
		var stopwatch = new Stopwatch();
		...
		try
		{
			...
			Assert.All(firstWave, received => Assert.True(received, $"An MQTT subscriber did not receive the complete {FIRST_WAVE_TOTAL}-message first wave."));
			...
			for(int index = 1; index < SUBSCRIBER_COUNT; index++)
				Assert.True(await audits[index].WaitForCountAsync(ALL_WAVES_TOTAL, TEST_TIMEOUT),
					$"MQTT subscriber {index} did not receive all {ALL_WAVES_TOTAL} messages while subscriber 0 unsubscribed.");
			...
			for(int index = 1; index < SUBSCRIBER_COUNT; index++)
				Assert.True(await audits[index].WaitForCountAsync(FINAL_TOTAL, TEST_TIMEOUT),
					$"MQTT subscriber {index} did not receive the post-unsubscribe probe.");
			...
			Assert.InRange(stopwatch.Elapsed, TimeSpan.Zero, TEST_TIMEOUT);
			Assert.True(throughput >= MINIMUM_DELIVERIES_PER_SECOND,
				$"MQTT unsubscribe throughput {throughput:F2} active deliveries/s was below {MINIMUM_DELIVERIES_PER_SECOND:F2}; {TIMED_DELIVERY_COUNT} deliveries took {stopwatch.Elapsed}.");
		}
		finally
		{
			await CleanupAsync(publishers, subscribers, consumers);
	...
	#region 私有方法
	private static async Task<string[]> PublishBatchAsync(MqttQueue queue, string topic, string prefix, int publisher, int count)
	{
	...
	private static Task<string>[] CreateGatedPublications(MqttQueue[] queues, string topic, string prefix, int count, Task gate) =>
	...
	private static async Task UnsubscribeAsync(MqttSubscriber subscriber, Task gate)
	{
		await gate;
	...
	private static async Task CleanupAsync(MqttQueue[] publishers, MqttQueue[] subscribers, MqttSubscriber[] consumers)
	{
		try
		{
	...
	private static void AssertReleased(MqttQueue[] queues, MqttSubscriber[] consumers)
	{
	...
	#region 嵌套类型
	private sealed class MqttMessageAudit(string topic) : HandlerBase<Message>
	{
		...
		public Task<bool> WaitForCountAsync(int count, TimeSpan timeout) =>
			MqttTestUtility.WaitUntilAsync(() => this.Count >= count, timeout);
