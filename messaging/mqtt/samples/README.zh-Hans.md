...
[server/Program.cs](server/Program.cs) 启动嵌入式代理，[client/Program.cs](client/Program.cs) 持有诊断用队列。应用侧采用[插件与公共队列提供者方式](../README.zh-Hans.md)。发布前等待订阅完成，取消订阅前查看接收输出；打印发送结果本身不能证明应用已处理消息或业务效果恰好执行一次。

🚨 使用专用本地代理并隔离监听端口，仅在唯一测试主题上发布虚拟数据。范例启动过程不配置生产身份认证、TLS 策略或持久消息存储。

客户端取消订阅后执行 `close`，再执行 `exit` 并确认；示例服务端执行 `stop`、`exit`。如果改为连接外部代理，客户端退出不会删除代理的保留消息或持久会话：仅清理本次拥有的测试资源，不要清空共享代理。
