...
partial class SequenceCommand
{
	...
	public class DecreaseCommand : CommandBase<CommandContext>
	{
		...
		#region 构造函数
		public DecreaseCommand() : base("Decrease") { }
		public DecreaseCommand(string name) : base(name) { }
		#endregion
		...
		protected override async ValueTask<object> OnExecuteAsync(CommandContext context, CancellationToken cancellation)
		{
			...
			if(round > 1)
			{
				...
				{
					var history = await DecreaseAsync(
						context.Output,
						sequence,
						context.Arguments[i],
						interval,
		...
		#region 私有方法
		static async ValueTask<long> DecreaseAsync(ICommandOutlet output, ISequenceBase sequence, string key, int interval, int seed, CancellationToken cancellation)
		{
			var value = await sequence.DecreaseAsync(key, interval, seed, cancellation);
		...
		static async ValueTask<ConcurrentBag<long>> DecreaseAsync(ICommandOutlet output, ISequenceBase sequence, string key, int interval, int seed, int round, TimeSpan jitter, CancellationToken cancellation)
		{
			...
			output.WriteLine(CommandOutletContent.Create()
				.Append(CommandOutletColor.DarkGray, ">> ")
				.Append(CommandOutletColor.DarkCyan, $"Completed {round} rounds for '{key}' in {stopwatch.Elapsed.TotalMilliseconds:F0} ms."));
