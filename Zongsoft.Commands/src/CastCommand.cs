...
public class CastCommand : CommandBase<CommandContext>
{
	...
	#region 私有方法
	private static BinaryReader GetReader(object source, Encoding encoding)
	{
		MemoryStream ms;
	...
	private static bool Move(BinaryReader reader, int offset)
	{
		try
		{
	...
	private static string ConvertToRawString(BinaryReader reader, int offset, int count, Encoding encoding, string[] args)
	{
		...
		if(offset > 0 && (!Move(reader, offset)))
			return null;
	...
	private static string ConvertToBase64(BinaryReader reader, int offset, int count, string[] args)
	{
		var buffer = new byte[BUFFER_SIZE];
		var bufferRead = 0;
		var text = new StringBuilder();
	...
	private static string ConvertToHexString(BinaryReader reader, int offset, int count, string[] args)
	{
		var buffer = new byte[BUFFER_SIZE];
		var bufferRead = 0;
		var total = 0;
